...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.num_heads_upsample = (
            num_heads_upsample if num_heads_upsample != -1 else num_heads
        )
        ...
        self.out = nn.Sequential(
            ...
            zero_module(
                conv_nd(
                    self._dims, self.model_channels, self.out_channels, 3, padding=1
                )
            ),
        )
        ...
        self.input_blocks = nn.ModuleList(
            [
                ContextAwareSequential(
                    conv_nd(
                        self._dims, self.in_channels, self.model_channels, 3, padding=1
                    )
                )
            ]
        )
